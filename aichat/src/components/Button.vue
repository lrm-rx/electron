<template>
  <button
    class="lk-button shadow-sm inline-flex items-center justify-center disabled:opacity-50 disabled:pointer-events-none"
...
export type ButtonColor = "violet" | "purple";
...
const colorVariants: Record<ButtonColor, any> = {
  violet: {
    plain:
      "bg-violet-50 text-violet-600 hover:bg-violet-600 border border-violet-600 hover:text-white",
    normal:
      "bg-violet-600 text-white hover:bg-violet-600/90 border border-violet-600",
  },
  purple: {
    plain:
...
const colorClasses = computed(() => {
  return props.plain
    ? colorVariants[props.color].plain
    : colorVariants[props.color].normal;
});
